...
// 名前空間を作成して競合を避ける
window.GridDisplay = window.GridDisplay || {};

(function(GridDisplay) {
    'use strict';
    
    // グローバル変数（GridDisplay名前空間内）
    GridDisplay.allGridPoints = null;
    GridDisplay.isLoadingAllGridPoints = false;
    GridDisplay.gridPointsLayer = null;
    GridDisplay.allGridPointsLayer = null;
    GridDisplay.usedGridPointsLayer = null;
    GridDisplay.currentDensity = 'auto';
    GridDisplay.autoMode = true;
...
function initializeMap() {
    ...
    map.on('click', function(e) {
        if (document.getElementById('spatial_method') && document.getElementById('spatial_method').value === 'point') {
...
function showAlert(type, message) {
    ...
    // (No unmatched catch/finally block here; remove this misplaced block)
})(window.GridDisplay);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 格子点表示のイベントリスナー追加（既存のものと重複しないようにチェック）
    if (showGridElement && !showGridElement.dataset.listenerAdded) {
        showGridElement.addEventListener('change', onGridPointsDisplayChange);
        showGridElement.dataset.listenerAdded = 'true';
        console.log('Grid points display event listener added');
    }
    
    // 格子密度選択のイベントリスナー
    if (densitySelectElement && !densitySelectElement.dataset.listenerAdded) {
        ...
        densitySelectElement.dataset.listenerAdded = 'true';
        console.log('Grid density event listener added');
    }
    
    console.log('External script initialization completed');
});
...
function displayAllGridPoints() {
    ...
    if (densitySelect && densitySelect.value !== 'auto') {
        ...
        console.log(`Manual density: step=${step} (${densitySelect.value})`);
    } else {
        ...
        console.log(`Auto density: step=${step} (zoom=${currentZoom})`);
    }
